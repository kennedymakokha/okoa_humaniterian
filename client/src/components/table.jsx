...
const Table = ({ columns, linkto, noExport, noAdd, editOnly, otherAction, noAction, notLinkable, enrolUser, data, setFilter, notdetailed, page, title, paginate, filter, isLoading, key_column, setShow, setItem, setPopUp }) => {
        ...
        <div className="py-10 ">
            <div className="flex w-full bg-[#98daf8] shadow-2xl min-w-[70hw] overflow-hidden items-center p-2 justify-between">
                ...
                {!notdetailed && <div className="flex items-center justify-center gap-x-3 ">
                    ...
                    {!noExport && <div onClick={() => exportToExcel(data, title)} className='px-4 text-[18px] shadow-2xl h-8 bg-slate-500 flex items-center justify-center font-bold text-white rounded-md'>Export</div>}
                        ...
                        {data?.map((row, rowIndex) => (
                            <tr key={rowIndex} className="even:bg-gray-50 ">
                                {columns.map((column) => (
                                    ...
                                    >
                                        <Link
                                            to={notLinkable ? "" : linkto ? linkto : `${location.pathname}/${row[key_column].replace(/\s+/g, '-')?.toLowerCase()}`
                                            } state={{ details: row }}
                                        >{column.accessor === "createdAt" ? `${moment(row[column.accessor]).format("Do MMM YYY hh:mm")}` : row[column.accessor]}</Link>
                                    </td>
                                ))}
                                ...
                                {!noAction && <td className="py-2 text-start  px-4 border-b border-l border-gray-200  text-gray-800">
                                    <div className="flex gap-2 ">
                                        {!editOnly && <div onClick={() => { setShow(true); setItem(row); }} className=' w-14 border-red-500  p-2 border  flex items-center justify-center rounded-md'>
